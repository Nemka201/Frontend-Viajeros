...
@Component({
  ...
  templateUrl: './add-post-view.component.html',
  styleUrls: ['./add-post-view.component.css'],
})
export class AddPostViewComponent {
  postForm: FormGroup;
  files: File[] = [];
  constructor(
  ...
  addPost() {
    ...
    try {
      this.cloudinaryService
        .uploadMultipleImages(this.files)
        .subscribe((imagesUrl) => {
          const httpsImagesUrl = imagesUrl.map((url) =>
            this.convertToHttps(url)
          );
